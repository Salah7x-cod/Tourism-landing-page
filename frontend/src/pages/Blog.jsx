...
import { PenLine, Upload, Link as LinkIcon, ChevronDown, ChevronUp, Loader2, CheckCircle2 } from "lucide-react";
...
export default function Blog() {
  ...
  useEffect(() => {
    api.listBlogs().then(setApiStories).catch(() => {});
    api.destinations().then(setDestinations).catch(() => {});
  }, []);

  const allStories = [...apiStories, ...fallbackStories];
  ...
  return (
    <div className="py-20 bg-background max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="text-center mb-16">
        <h1 className="text-5xl font-serif text-foreground mb-4 uppercase tracking-[0.1em]">
          {t("blog.title")}
        </h1>
        ...
        <div className="mb-12">
          ...
          {showForm && (
            <form
              ...
              className="mt-6 max-w-2xl mx-auto bg-card border border-border rounded-2xl p-8 shadow-xl space-y-5 animate-fade-in"
            >
              <div>
              ...
              <div>
                ...
                <div className="flex gap-3 items-center">
                  <label className="flex items-center gap-2 px-4 py-2 bg-secondary rounded-lg cursor-pointer hover:bg-secondary/80 transition-colors text-sm font-medium text-foreground">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.files?.[0]) setFormData({ ...formData, cover_image: "" });
                      }}
                    />
                  </label>
                  <span className="text-muted-foreground text-xs">{t("blog.orPasteUrl")}</span>
                ...
                {imageFile && (
                  <p className="text-xs text-primary mt-1.5">📎 {imageFile.name}</p>
                )}
              </div>
      ...
      <div className="space-y-24">
        {allStories.map((story) => {
          ...
          const coverImg = story.cover_image;
            ...
            <article key={story.id} className="border-b border-border/50 pb-24 last:border-0 last:pb-0">
              ...
              <div className="prose prose-lg prose-invert max-w-none text-muted-foreground grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-16 items-start">
                <div className="md:col-span-7 space-y-6">
                  {paragraphs[0] && (
                    <p className="text-2xl leading-relaxed text-foreground font-light">{paragraphs[0]}</p>
                  )}
