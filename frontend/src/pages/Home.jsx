...
export default function Home() {
  ...
  const featuredDestinations = total > 0 
    ? [...destinations, ...destinations, ...destinations].slice(startIndex, startIndex + 3) 
    : [];

  const handleNext = () => total > 0 && setStartIndex((prev) => (prev + 1) % total);
  const handlePrev = () => total > 0 && setStartIndex((prev) => (prev - 1 + total) % total);
        ...
        <div className="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {featuredDestinations.map((dest, idx) => (
              <Link to={`/explore/${dest.id}`} key={`${dest.id}-${idx}`} className="group relative h-[450px] w-full rounded-3xl overflow-hidden cursor-pointer block">
                <img 
                  src={dest.image} 
                  alt={dest.name} 
          ...
          <div className="flex justify-center mt-12 gap-4">
             <button onClick={handlePrev} aria-label="Previous" className="w-10 h-10 rounded-full border-2 border-[#013220]/30 bg-white flex items-center justify-center text-[#013220] hover:bg-[#013220] hover:text-white hover:border-[#013220] transition-colors shadow-sm active:scale-95">
               <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
             </button>
             <button onClick={handleNext} aria-label="Next" className="w-10 h-10 rounded-full border-2 border-[#013220]/30 bg-white flex items-center justify-center text-[#013220] hover:bg-[#013220] hover:text-white hover:border-[#013220] transition-colors shadow-sm active:scale-95">
